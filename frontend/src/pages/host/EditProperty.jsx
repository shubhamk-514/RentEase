...
const EditProperty = ({ showToast }) => {
  const { id } = useParams();
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const { currentRoom, detailLoading, actionLoading, success } = useSelector(
    (state) => state.rooms,
  );
  // Form states
  ...
  // Fill form once room loaded
  useEffect(() => {
    if (currentRoom) {
      setTitle(currentRoom.title || "");
      setDescription(currentRoom.description || "");
      setPrice(currentRoom.price || "");
      setCategory(currentRoom.category || "room");
      setLocation(currentRoom.location || "");
      setAddress(currentRoom.address || "");
      setContactNumber(currentRoom.contactNumber || "");
      setAvailabilityStatus(currentRoom.availabilityStatus || "available");
      setSelectedAmenities(currentRoom.amenities || []);
      setRules(currentRoom.rules || []);
      setImages(currentRoom.images || []);
    }
  }, [currentRoom]);
  // Redirect on success
  useEffect(() => {
  ...
  const handleSubmit = (e) => {
    ...
    dispatch(
      ...
      .unwrap()
      .catch((err) => {
        showToast(err || "Failed to update listing", "error");
      });
  };
  if (detailLoading) {
    return (
    ...
    <div className="max-w-4xl mx-auto px-4 py-10">
      <div className="mb-8 space-y-2">
        <h1 className="text-3xl font-black font-sans flex items-center gap-2">
          <EditIcon className="w-8 h-8 text-indigo-400" /> Edit Listing
        </h1>
        <p className="text-slate-400 text-sm">
          Update pricing, availability status, description parameters, or upload
          more photos.
        </p>
      </div>
      <form
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="md:col-span-2">
              ...
              <textarea
                required
                rows={4}
                placeholder="Detail room metrics..."
                value={description}
...
// Inline Edit icon since it was not imported
